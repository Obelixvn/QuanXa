<?
    ...
    $sql = "
        ...
        Where   tb_boi_hour.Date >= '".$date->format('Y-m-d')."' and
                tb_boi_hour.Date <= '".$date->modify('+6 day')->format('Y-m-d')."'
    ...
    <tbody>
        ...
        <?php
            ...
            for ($i=1; $i <= count($ten_boi); $i++) {
                ...
                $tien_luong = $luong_boi[$i]* 7.2;
                $tong_tienLuong += $tien_luong;
                echo "<td>".money_format('%#10.2n',$tien_luong)."</td>";
                echo "<td><button type =\"button\" onclick = \"show_lichLam_detailt('".$ten_boi[$i]."','".$date->format('Y-m-d')."')\">Chi tiet</button></td>";
                echo "</tr>";
